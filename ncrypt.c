...
#define WIPE_CTX(ctx)              ncrypt_wipe(ctx   , sizeof(*(ctx)))
#define WIPE_BUFFER(buffer)        ncrypt_wipe(buffer, sizeof(buffer))
...
int ncrypt_verify16(const u8 a[16], const u8 b[16]){ return neq0(x16(a, b)); }
int ncrypt_verify32(const u8 a[32], const u8 b[32]){ return neq0(x32(a, b)); }
int ncrypt_verify64(const u8 a[64], const u8 b[64]){ return neq0(x64(a, b)); }
...
void ncrypt_chacha20_h(u8 out[32], const u8 key[32], const u8 in [16])
{
	u32 block[16];
...
u64 ncrypt_chacha20_djb(u8 *cipher_text, const u8 *plain_text,
...
u32 ncrypt_chacha20_ietf(u8 *cipher_text, const u8 *plain_text,
                         size_t text_size,
                         const u8 key[32], const u8 nonce[12], u32 ctr)
{
	u64 big_ctr = ctr + ((u64)load32_le(nonce) << 32);
	return (u32)ncrypt_chacha20_djb(cipher_text, plain_text, text_size,
	                                key, nonce + 4, big_ctr);
}

u64 ncrypt_chacha20_x(u8 *cipher_text, const u8 *plain_text,
                      size_t text_size,
                      const u8 key[32], const u8 nonce[24], u64 ctr)
{
	u8 sub_key[32];
	ncrypt_chacha20_h(sub_key, key, nonce);
	ctr = ncrypt_chacha20_djb(cipher_text, plain_text, text_size,
...
// Postcondition:
//   ctx->h <= 4_ffffffff_ffffffff_ffffffff_ffffffff
static void poly_blocks(ncrypt_poly1305_ctx *ctx, const u8 *in,
...
void ncrypt_poly1305_init(ncrypt_poly1305_ctx *ctx, const u8 key[32])
{
...
void ncrypt_poly1305_final(ncrypt_poly1305_ctx *ctx, u8 mac[16])
{
...
void ncrypt_poly1305(u8     mac[16],  const u8 *message,
                     size_t message_size, const u8  key[32])
{
	ncrypt_poly1305_ctx ctx;
	ncrypt_poly1305_init  (&ctx, key);
	ncrypt_poly1305_update(&ctx, message, message_size);
	ncrypt_poly1305_final (&ctx, mac);
}
...
static void blake2b_compress(ncrypt_blake2b_ctx *ctx, int is_last_block)
{
                               ...
                               const u8 *key, size_t key_size)
{
	...
	if (key_size > 0) {
		...
		// same as calling ncrypt_blake2b_update(ctx, key_block , 128)
...
{
	ncrypt_blake2b_keyed_init(ctx, hash_size, 0, 0);
}
...
void ncrypt_blake2b_final(ncrypt_blake2b_ctx *ctx, u8 *hash)
{
...
void ncrypt_blake2b_keyed(u8 *hash,          size_t hash_size,
                          const u8 *key,     size_t key_size,
                          const u8 *message, size_t message_size)
{
	ncrypt_blake2b_ctx ctx;
	ncrypt_blake2b_keyed_init(&ctx, hash_size, key, key_size);
	ncrypt_blake2b_update    (&ctx, message, message_size);
	ncrypt_blake2b_final     (&ctx, hash);
}

void ncrypt_blake2b(u8 *hash, size_t hash_size, const u8 *msg, size_t msg_size)
{
	ncrypt_blake2b_keyed(hash, hash_size, 0, 0, msg, msg_size);
}
...
static void blake_update_32(ncrypt_blake2b_ctx *ctx, u32 input)
{
	u8 buf[4];
	store32_le(buf, input);
	ncrypt_blake2b_update(ctx, buf, 4);
	WIPE_BUFFER(buf);
}

static void blake_update_32_buf(ncrypt_blake2b_ctx *ctx,
                                const u8 *buf, u32 size)
{
	blake_update_32(ctx, size);
	ncrypt_blake2b_update(ctx, buf, size);
}
...
{
	ncrypt_blake2b_ctx ctx;
	ncrypt_blake2b_init  (&ctx, MIN(digest_size, 64));
	blake_update_32      (&ctx, digest_size);
	ncrypt_blake2b_update(&ctx, input, input_size);
	ncrypt_blake2b_final (&ctx, digest);
	...
	if (digest_size > 64) {
		...
		while (i < r) {
			...
			ncrypt_blake2b(digest + out, 64, digest + in, 64);
			i   +=  1;
			in  += 32;
			out += 32;
		}
		ncrypt_blake2b(digest + out, digest_size - (32 * r), digest + in , 64);
	}
}
...
const ncrypt_argon2_extras ncrypt_argon2_no_extras = { 0, 0, 0, 0 };

void ncrypt_argon2(u8 *hash, u32 hash_size, void *work_area,
                   ...
                   ncrypt_argon2_extras extras)
{
	...
	blk *blocks = (blk*)work_area;
	{
		...
		ncrypt_blake2b_final(&ctx, initial_hash); // fill 64 first bytes only
	...
	int constant_time = config.algorithm != NCRYPT_ARGON2_D;
	...
	FOR_T(u32, pass, 0, config.nb_passes) {
		FOR_T(u32, slice, 0, 4) {
			...
			if (slice == 2 && config.algorithm == NCRYPT_ARGON2_ID) {
				constant_time = 0;
			}
...
static int fe_isequal(const fe f, const fe g)
{
	...
	fe_tobytes(fs, f);
	fe_tobytes(gs, g);
	int isdifferent = ncrypt_verify32(fs, gs);
	WIPE_BUFFER(fs);
	WIPE_BUFFER(gs);
...
void ncrypt_eddsa_trim_scalar(u8 out[32], const u8 in[32])
{
	COPY(out, in, 32);
	out[ 0] &= 248;
	out[31] &= 127;
...
void ncrypt_x25519(u8       raw_shared_secret[32],
                   const u8 your_secret_key  [32],
                   const u8 their_public_key [32])
{
	// restrict the possible scalar values
	u8 e[32];
	ncrypt_eddsa_trim_scalar(e, your_secret_key);
	scalarmult(raw_shared_secret, e, their_public_key, 255);
	WIPE_BUFFER(e);
}

void ncrypt_x25519_public_key(u8       public_key[32],
                              const u8 secret_key[32])
{
	static const u8 base_point[32] = {9};
	ncrypt_x25519(public_key, secret_key, base_point);
}
...
void ncrypt_eddsa_reduce(u8 reduced[32], const u8 expanded[64])
{
	u32 x[16];
...
// r = (a * b) + c
void ncrypt_eddsa_mul_add(u8 r[32],
...
int ncrypt_eddsa_check_equation(const u8 signature[64], const u8 public_key[32],
                                const u8 h[32])
{
	...
	return ncrypt_verify32(check, zero_point);
}
...
static void ge_scalarmult_base(ge *p, const u8 scalar[32])
{
	...
	u8 s_scalar[32];
	ncrypt_eddsa_mul_add(s_scalar, scalar, half_mod_L, half_ones);
...
void ncrypt_eddsa_scalarbase(u8 point[32], const u8 scalar[32])
{
	ge P;
...
void ncrypt_eddsa_key_pair(u8 secret_key[64], u8 public_key[32], u8 seed[32])
{
	...
	ncrypt_blake2b(a, 64, a, 32);
	ncrypt_eddsa_trim_scalar(a, a);
	ncrypt_eddsa_scalarbase(secret_key + 32, a);
                        ...
                        const u8 *c, size_t c_size)
{
	...
	ncrypt_blake2b_ctx ctx;
	ncrypt_blake2b_init  (&ctx, 64);
	ncrypt_blake2b_update(&ctx, a, a_size);
	ncrypt_blake2b_update(&ctx, b, b_size);
	ncrypt_blake2b_update(&ctx, c, c_size);
	ncrypt_blake2b_final (&ctx, hash);
	ncrypt_eddsa_reduce(h, hash);
}
...
//   S              = ((h * a) + r) % L
//   signature      = R || S
void ncrypt_eddsa_sign(u8 signature [64], const u8 secret_key[64],
                       const u8 *message, size_t message_size)
{
...
//
// The last two steps are done in ncrypt_eddsa_check_equation()
int ncrypt_eddsa_check(const u8  signature[64], const u8 public_key[32],
                       const u8 *message, size_t message_size)
{
	u8 h[32];
	hash_reduce(h, signature, 32, public_key, 32, message, message_size);
	return ncrypt_eddsa_check_equation(signature, public_key, h);
}

/////////////////////////
/// EdDSA <--> X25519 ///
/////////////////////////
void ncrypt_eddsa_to_x25519(u8 x25519[32], const u8 eddsa[32])
{
...
void ncrypt_x25519_to_eddsa(u8 eddsa[32], const u8 x25519[32])
{
...
// with ncrypt_curve_to_hidden().
//
// The public key is otherwise compatible with ncrypt_x25519(), which
...
// representatives (once we apply ncrypt_curve_to_hidden()).
//
...
void ncrypt_x25519_dirty_small(u8 public_key[32], const u8 secret_key[32])
{
	...
	// to yield the same results as ncrypt_x25519_dirty_fast().
	...
	u8 scalar[32];
	ncrypt_eddsa_trim_scalar(scalar, secret_key);
...
void ncrypt_x25519_dirty_fast(u8 public_key[32], const u8 secret_key[32])
{
...
//       u2 = w * non_square * r^2
//       u2 = u
void ncrypt_elligator_map(u8 curve[32], const u8 hidden[32])
{
	fe r, u, t1, t2, t3;
...
int ncrypt_elligator_rev(u8 hidden[32], const u8 public_key[32], u8 tweak)
{
	fe t1, t2, t3;
...
void ncrypt_elligator_key_pair(u8 hidden[32], u8 secret_key[32], u8 seed[32])
{
	u8 pk [32]; // public key
	u8 buf[64]; // seed + representative
	COPY(buf + 32, seed, 32);
	do {
		ncrypt_chacha20_djb(buf, 0, 64, buf+32, zero, 0);
		ncrypt_x25519_dirty_fast(pk, buf); // or the "small" version
	} while(ncrypt_elligator_rev(buf+32, pk, buf[32]));
	// Note that the return value of ncrypt_elligator_rev() is
...
void ncrypt_x25519_inverse(u8 blind_salt [32], const u8 private_key[32],
                           const u8 curve_point[32])
{
	...
	u8 scalar[32];
	ncrypt_eddsa_trim_scalar(scalar, private_key);
                      ...
                      const u8 *cipher_text, size_t text_size)
{
	...
	ncrypt_poly1305_ctx poly_ctx;           // auto wiped...
	ncrypt_poly1305_init  (&poly_ctx, auth_key);
	ncrypt_poly1305_update(&poly_ctx, ad         , ad_size);
	ncrypt_poly1305_update(&poly_ctx, zero       , gap(ad_size, 16));
	ncrypt_poly1305_update(&poly_ctx, cipher_text, text_size);
	ncrypt_poly1305_update(&poly_ctx, zero       , gap(text_size, 16));
	ncrypt_poly1305_update(&poly_ctx, sizes      , 16);
	ncrypt_poly1305_final (&poly_ctx, mac); // ...here
}

void ncrypt_aead_init_x(ncrypt_aead_ctx *ctx,
                        u8 const key[32], const u8 nonce[24])
{
	ncrypt_chacha20_h(ctx->key, key, nonce);
...
void ncrypt_aead_write(ncrypt_aead_ctx *ctx, u8 *cipher_text, u8 mac[16],
                       const u8 *ad,         size_t ad_size,
                       const u8 *plain_text, size_t text_size)
{
	u8 auth_key[64]; // the last 32 bytes are used for rekeying.
	ncrypt_chacha20_djb(auth_key, 0, 64, ctx->key, ctx->nonce, ctx->counter);
	ncrypt_chacha20_djb(cipher_text, plain_text, text_size,
...
int ncrypt_aead_read(ncrypt_aead_ctx *ctx, u8 *plain_text, const u8 mac[16],
                     const u8 *ad,          size_t ad_size,
                     const u8 *cipher_text, size_t text_size)
{
	u8 auth_key[64]; // the last 32 bytes are used for rekeying.
	...
	int mismatch = ncrypt_verify16(mac, real_mac);
	if (!mismatch) {
		ncrypt_chacha20_djb(plain_text, cipher_text, text_size,
		                    ctx->key, ctx->nonce, ctx->counter + 1);
		COPY(ctx->key, auth_key + 32, 32);
	}
	WIPE_BUFFER(auth_key);
...
void ncrypt_aead_lock(u8 *cipher_text, u8 mac[16], const u8 key[32],
                      const u8  nonce[24], const u8 *ad, size_t ad_size,
                      const u8 *plain_text, size_t text_size)
{
	ncrypt_aead_ctx ctx;
	ncrypt_aead_init_x(&ctx, key, nonce);
	ncrypt_aead_write(&ctx, cipher_text, mac, ad, ad_size,
	                  plain_text, text_size);
	ncrypt_wipe(&ctx, sizeof(ctx));
}

int ncrypt_aead_unlock(u8 *plain_text, const u8  mac[16], const u8 key[32],
                       const u8 nonce[24], const u8 *ad, size_t ad_size,
                       const u8 *cipher_text, size_t text_size)
{
	ncrypt_aead_ctx ctx;
	ncrypt_aead_init_x(&ctx, key, nonce);
	int mismatch = ncrypt_aead_read(&ctx, plain_text, mac, ad, ad_size,
...
static void sha512_compress(ncrypt_sha512_ctx *ctx)
{
...
// Write 1 input byte
static void sha512_set_input(ncrypt_sha512_ctx *ctx, u8 input)
{
	size_t word = ctx->input_idx >> 3;
	size_t byte = ctx->input_idx &  7;
...
void ncrypt_sha512_final(ncrypt_sha512_ctx *ctx, u8 hash[64])
{
	// Add padding bit
	if (ctx->input_idx == 0) {
		ZERO(ctx->input, 16);
...
void ncrypt_sha512(u8 hash[64], const u8 *message, size_t message_size)
{
	ncrypt_sha512_ctx ctx;
	ncrypt_sha512_init  (&ctx);
	ncrypt_sha512_update(&ctx, message, message_size);
	ncrypt_sha512_final (&ctx, hash);
}
                             ...
                             const u8 *key, size_t key_size)
{
	...
	if (key_size > 128) {
		ncrypt_sha512(ctx->key, key, key_size);
		key      = ctx->key;
		key_size = 64;
	}
	...
	ncrypt_sha512_init  (&ctx->ctx);
	ncrypt_sha512_update(&ctx->ctx, ctx->key, 128);
}

void ncrypt_sha512_hmac_update(ncrypt_sha512_hmac_ctx *ctx,
                               const u8 *message, size_t message_size)
{
	ncrypt_sha512_update(&ctx->ctx, message, message_size);
}

void ncrypt_sha512_hmac_final(ncrypt_sha512_hmac_ctx *ctx, u8 hmac[64])
{
	// Finish computing inner hash
	ncrypt_sha512_final(&ctx->ctx, hmac);
	...
	ncrypt_sha512_init  (&ctx->ctx);
	ncrypt_sha512_update(&ctx->ctx, ctx->key , 128);
	ncrypt_sha512_update(&ctx->ctx, hmac, 64);
	ncrypt_sha512_final (&ctx->ctx, hmac); // outer hash
	WIPE_CTX(ctx);
}

void ncrypt_sha512_hmac(u8 hmac[64], const u8 *key, size_t key_size,
                        const u8 *message, size_t message_size)
{
	ncrypt_sha512_hmac_ctx ctx;
	ncrypt_sha512_hmac_init  (&ctx, key, key_size);
	ncrypt_sha512_hmac_update(&ctx, message, message_size);
	ncrypt_sha512_hmac_final (&ctx, hmac);
}

////////////////////
/// HKDF SHA 512 ///
////////////////////
void ncrypt_sha512_hkdf_expand(u8       *okm,  size_t okm_size,
                        ...
                        const u8 *info, size_t info_size)
{
	// Extract
	u8 prk[64];
	ncrypt_sha512_hmac(prk, salt, salt_size, ikm, ikm_size);

	// Expand
	ncrypt_sha512_hkdf_expand(okm, okm_size, prk, sizeof(prk), info, info_size);
...
/// Ed25519 ///
///////////////
void ncrypt_ed25519_key_pair(u8 secret_key[64], u8 public_key[32], u8 seed[32])
{
	u8 a[64];
	COPY(a, seed, 32);                      // a[ 0..31]  = seed
	ncrypt_wipe(seed, 32);
	COPY(secret_key, a, 32);                // secret key = seed
	ncrypt_sha512(a, a, 32);                // a[ 0..31]  = scalar
	ncrypt_eddsa_trim_scalar(a, a);         // a[ 0..31]  = trimmed scalar
	ncrypt_eddsa_scalarbase(public_key, a); // public key = [trimmed scalar]B
	COPY(secret_key + 32, public_key, 32);  // secret key includes public half
	WIPE_BUFFER(a);
}

static void sha512_hash_reduce(u8 h[32],
                        const u8 *a, size_t a_size,
                        const u8 *b, size_t b_size,
                        const u8 *c, size_t c_size,
                        const u8 *d, size_t d_size)
{
	...
	ncrypt_sha512_ctx ctx;
	ncrypt_sha512_init  (&ctx);
	ncrypt_sha512_update(&ctx, a, a_size);
	ncrypt_sha512_update(&ctx, b, b_size);
	ncrypt_sha512_update(&ctx, c, c_size);
	ncrypt_sha512_update(&ctx, d, d_size);
	ncrypt_sha512_final (&ctx, hash);
	ncrypt_eddsa_reduce(h, hash);
}
                             ...
                             const u8 *message, size_t message_size)
{
	...
	ncrypt_sha512(a, secret_key, 32);
	ncrypt_eddsa_trim_scalar(a, a);
	sha512_hash_reduce(r, dom, dom_size, a + 32, 32, message, message_size, 0, 0);
	ncrypt_eddsa_scalarbase(R, r);
	sha512_hash_reduce(h, dom, dom_size, R, 32, pk, 32, message, message_size);
	COPY(signature, R, 32);
	ncrypt_eddsa_mul_add(signature + 32, h, a, r);
...
void ncrypt_ed25519_sign(u8 signature [64], const u8 secret_key[64],
                         const u8 *message, size_t message_size)
{
	ed25519_dom_sign(signature, secret_key, 0, 0, message, message_size);
}

int ncrypt_ed25519_check(const u8 signature[64], const u8 public_key[32],
                         const u8 *msg, size_t msg_size)
{
	u8 h_ram[32];
	sha512_hash_reduce(h_ram, signature, 32, public_key, 32, msg, msg_size, 0, 0);
	return ncrypt_eddsa_check_equation(signature, public_key, h_ram);
}

static const u8 domain[34] = "SigEd25519 no Ed25519 collisions\1";

void ncrypt_ed25519_ph_sign(uint8_t signature[64], const uint8_t secret_key[64],
...
int ncrypt_ed25519_ph_check(const uint8_t sig[64], const uint8_t pk[32],
                            const uint8_t msg_hash[64])
{
	u8 h_ram[32];
	sha512_hash_reduce(h_ram, domain, sizeof(domain), sig, 32, pk, 32, msg_hash, 64);
	return ncrypt_eddsa_check_equation(sig, pk, h_ram);
}
